...
pub fn run_backup(source: &Path, destination: &Path, chunk_size: usize) -> Result<()> {
    ...
    let store = Arc::new(LocalStore::open(destination)
        .context("Failed to open local store")?);

    let index: Arc<dyn DedupIndex> = Arc::new(HashIndex::new(1_000_000));
    store.populate_index(&index).context("Failed to populate index from store")?;
    ...
    println!("  Dedup saved:     {} bytes ({:.1}%)", stats.dedup_size, stats.space_savings_pct());
