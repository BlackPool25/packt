...
pub fn test_backup_env(corpus: &[u8], chunk_size: usize) -> (TempDir, BackupPipeline, Arc<packt_lib::store::local::LocalStore>) {
    ...
    let store = Arc::new(
        LocalStore::open(store_dir.path()).expect("Failed to open store"),
    );
